...
interface AdminLoginPageProps {
  onLogin: (email: string, securityKey: string) => Promise<boolean>;
}
...
export function AdminLoginPage({ onLogin }: AdminLoginPageProps) {
  ...
  const handleAuthenticate = async (e: React.FormEvent) => {
    ...
    // Real authentication
    try {
      const success = await onLogin(email, securityKey);
      if (!success) {
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full rounded-lg border ${isFocused === 'email'
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onBlur={() => setIsFocused(null)}
                  placeholder="••••••••••••••••"
                  className={`w-full rounded-lg border ${isFocused === 'securityKey'
                    ? 'border-cyan-500 ring-2 ring-cyan-500/20'
                    : 'border-slate-300'
                    } bg-white py-3 pl-11 pr-4 text-slate-900 placeholder-slate-400 transition-all duration-200 focus:outline-none`}
                  required
